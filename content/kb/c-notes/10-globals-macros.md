...
date: "2026-06-16"
category: "c-notes"
description: "全局变量、static变量、宏定义#define与多文件编程"
---
### 全局变量
#### 认识 全局变量
- 定义在函数外的变量就是全局变量
...
![？？？](/static/c/c-aHR0cHM6Ly9pbWct.jpg)
...
![1.创建一个 c文件](/static/c/c-aHR0cHM6Ly9pbWct.png)

![2.写一个简单的带宏的 c程序](/static/c/c-aHR0cHM6Ly9pbWct.png)

![3.这时后我们成功创建了一个 c文件](/static/c/c-aHR0cHM6Ly9pbWct.png)

![4.编译c文件，并保留中间文件](/static/c/c-aHR0cHM6Ly9pbWct.png)
现在多出来了 4 个文件，蓝色的是文件夹，我们不去管它，绿色的是可执行文件，类似 windows 的 .exe 文件
现在我们主要关注这 3 个中间文件

![文件详细情况](/static/c/c-aHR0cHM6Ly9pbWct.png)
一个 c文件编译的过程文件变化是这样的：
>`.c `（处理编译预处理指令）-> `.i `（产生汇编代码）-> `.s`（汇编生成目标文件） -> `.o`（链接等） -> `a.out `
>

可以看到 .i 文件时很大
![我们看看 .i 文件的 结尾部分](/static/c/c-aHR0cHM6Ly9pbWct.png)
![对比 .c 文件](/static/c/c-aHR0cHM6Ly9pbWct.png)
**我们发现程序中的宏 PI 被换成了它所表示的 数字**

这种替换是**简单的文本替换**，我们再试试其他的替换方式：

![替换字符串](/static/c/c-aHR0cHM6Ly9pbWct.png)
![FRORMAT 同样被替换了](/static/c/c-aHR0cHM6Ly9pbWct.png)
我们再试试这样，定义宏的时候 不带双引号：
![编译器给了 warning](/static/c/c-aHR0cHM6Ly9pbWct.png)
![FORMAT并没有被替换](/static/c/c-aHR0cHM6Ly9pbWct.png)
...
![](/static/c/c-aHR0cHM6Ly9pbWct.gif)
#### 带参数的宏
- `#define cube(x) ( (x) * (x) * (x) )`
...
![注意运算顺序](/static/c/c-aHR0cHM6Ly9pbWct.png)
**定义带参数的宏的原则**
- 一切都要有括号
   - 整个值有括号
   - 每个参数都有括号
