...
export default function PostHeroBanner({
  ...
  return (
    <div className="relative w-full h-[45vh] min-h-[250px] md:h-[55vh] md:min-h-[400px] max-h-[600px] mb-6 sm:mb-8 flex items-center justify-center overflow-hidden">
      ...
      {/* 内容信息层 */}
      <div className="relative z-10 w-full max-w-4xl px-4 sm:px-6 pt-10 sm:pt-16 flex flex-col items-center text-center space-y-4 sm:space-y-6">
        {/* 强制白字并赋予深邃光晕级阴影，这能在不污染背景图的情况下确保极致的可读性，符合高端海报排版 */}
        <PageTitle className="text-2xl sm:text-4xl md:text-5xl lg:text-[54px] leading-snug sm:leading-tight font-black tracking-tight text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.6)] dark:drop-shadow-[0_4px_16px_rgba(0,0,0,0.8)] px-2">
          {title}
        </PageTitle>
